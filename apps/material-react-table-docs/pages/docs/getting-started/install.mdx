...
<Head>
  <title>{'Installation - Material React Table V4 Docs'}</title>
  <meta name="description" content="How to install Material React Table" />
</Head>

## Installation

These are install instructions for the latest version of Material React Table (V4), published as `@lminii/material-react-table`
...
> `@lminii/material-react-table` requires **Material UI V9** packages as dependencies in your project.
>
...
>
> 1.  `@mui/material` (V9)
> 2.  `@mui/x-date-pickers` (V9)
> 3.  `@mui/icons-material` (V9)
> 4.  `@emotion/react` (v11.13+)
> 5.  `@emotion/styled` (v11.13+)
> 6.  `react` and `react-dom` (v18+)

### Quick Install

<InstallCommand packagesString="@lminii/material-react-table" />

### Install With Required Peer Dependencies (Recommended)

<InstallCommand />

> You do NOT need to install `@tanstack/react-table`, as it is already an internal dependency of `@lminii/material-react-table`, and must use an exact version already specified internally.
>
...
If `require()` of the package fails with `ERR_REQUIRE_ESM` or a similar error, you are running the CommonJS build on Node older than 22.12. TanStack Table V9 is ESM-only, so the CommonJS build of `@lminii/material-react-table` needs Node 22.12 or newer to `require()` ES modules. Upgrade Node or switch to ESM.
...
Or you probably do not have the correct version of Material UI installed. Make sure all Material UI packages are at least V9.0 or higher if you are using MRT V4 or higher.
